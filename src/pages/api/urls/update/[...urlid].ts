...
export default async function handler(
  ...
  if (!(req.method == "POST")) {
    res
      .status(400)
      .json({ error: "Bad request. This type of request is not allowed." });
    return;
  }
  ...
  if (!urlid || (urlid && typeof urlid == "object" && urlid.length > 1)) {
    res.status(400).json({
      error:
        "Bad request! updates should be to and only to /api/urls/update/urlid as a post request.",
    });
    return;
  }
  ...
  if (!user) {
    res.status(400).json({
      error:
        "Bad request! User not found. If you think this is a bug/mistake, please let me know at https://github.com/azeek21",
    });
    return;
  }
  ...
  if (!user.urls || !user.urls.includes(urlid)) {
    res
      .status(400)
      .json({ error: "Bad request! This url doesn't belong to this user." });
    return;
  }
  ...
  if (!url) {
    res.status(400).json({
      error:
        "Something went wrong. Plase report it to https://github.com/azeek21. error code: 1. Description: User urls includes url but url can't be found in db",
    });
    return;
  }
  ...
  if (data.timeout) {
    ...
    if (typeof data.timeout !== "string") {
      res.status(400).json({
        error:
          'Bad request. Expiration date must be a string in "YYYY-MM-DD" format.',
      });
      return;
    }
    ...
    if (!datetimeRegex.test(data.timeout)) {
      res.status(400).json({
        error:
          'Bad request. Expiration date doesn\'t match the needed format: "YYYY-MM-DD" ',
      });
      return;
    }
  }
  ...
  if (data.limit) {
    ...
    if (typeof data.limit != "string" && typeof data.limit != "number") {
      res.status(400).json({
        error:
          "Bad request. Limit must be a string of only numbers or an actual number. ",
      });
      return;
    }
  ...
  if (typeof data.password != "string") {
    res
      .status(400)
      .json({ error: "Bad request. Password must be a valid string" });
    return;
  }
