...
export default async function handler(req:NextApiRequest, res: NextApiResponse) {
    if (req.method !== "DELETE") {
        res.status(400).json({ error: "Bad request. This type of request not allowed."});
        return ;
    }
    ...
    res.status(400).json({ error: "FATAL! Something went wrong please report to https://github.com/azeek21 !"})
};
